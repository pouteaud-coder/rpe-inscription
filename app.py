            ...
            with st.expander(f"{format_date_fr_complete(at['date_atelier'])} — {at['titre']} | {statut}"):
                ...
                if c3.button("Valider", key=f"v_{at['id']}", type="primary"):
                    if qui != "Choisir...":
                        ...
                        diff = (nb_e - (existing['nb_enfants'] if existing else 0)) if existing else (1 + nb_e)
...
# ==========================================
    ...
    with t1:
        ...
        if data.data:
            df = pd.DataFrame([{"Assistante Maternelle": f"{i['adherents']['prenom']} {i['adherents']['nom']}", "Date": i['ateliers']['date_atelier'], "Titre": i['ateliers']['titre'], "Enfants": i['nb_enfants']} for i in data.data])
            c1, c2 = st.columns(2)
            c1.download_button("Excel", export_to_excel(df), "suivi_am.xlsx")
            c2.download_button("PDF", export_to_pdf("Suivi AM", [f"{r['Assistante Maternelle']} - {r['Date']} - {r['Titre']}" for r in df.to_dict('records')]), "suivi_am.pdf")
            curr = ""
            for i in data.data:
    ...
    with t2:
        ...
        for index, a in enumerate(ats.data):
            ...
            st.markdown(f"**{format_date_fr_complete(a['date_atelier'])}** | {a['titre']} <span class='compteur-badge'>👤 {t_am} AM</span> <span class='compteur-badge'>👶 {t_en} enf.</span>", unsafe_allow_html=True)
            if ins.data:
                html = "<div class='container-inscrits'>"
                for p in sorted(ins.data, key=lambda x: x['adherents']['nom']):
                    html += f'<span class="liste-inscrits">• {p["adherents"]["prenom"]} {p["adherents"]["nom"]} <span class="nb-enfants-focus">({p["nb_enfants"]} enf.)</span></span>'
                st.markdown(html + "</div>", unsafe_allow_html=True)
            if index < len(ats.data)-1: st.markdown('<hr class="separateur-atelier">', unsafe_allow_html=True)
...
# ==========================================
    ...
    if pw == current_code:
        ...
        with t1: # ATELIERS
            ...
            if sub == "Générateur":
                c1, c2 = st.columns(2)
                d1, d2 = c1.date_input("Début", date.today()), c2.date_input("Fin", date.today() + timedelta(days=7))
                ...
                if 'at_list_gen' in st.session_state:
                    df_ed = st.data_editor(pd.DataFrame(st.session_state['at_list_gen']), num_rows="dynamic", column_config={
                ...
                fs, fe = cf1.date_input("Du", date.today()-timedelta(days=60)), cf2.date_input("Au", date.today()+timedelta(days=60))
                ...
                for a in rep:
                    ...
                    c_a.write(f"{'🟢' if a['est_actif'] else '🔴'} **{format_date_fr_complete(a['date_atelier'])}** - {a['titre']} ({a['lieux']['nom']})")
                ...
                with st.form("bulk_form"):
                    bs, be = st.date_input("Début"), st.date_input("Fin")
