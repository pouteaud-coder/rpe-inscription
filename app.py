...
def format_date_fr_complete(date_val, gras=True):
    """Transforme une date (objet ou string ISO) en format français complet."""
...
def parse_date_fr_to_iso(date_str):
    """Sécurité : Nettoie les balises Markdown et reconvertit en ISO pour la DB."""
...
# --- DIALOGUES ---
@st.dialog("⚠️ Confirmation Suppression")
def secure_delete_dialog(table, item_id, label, current_code):
    st.write(f"Voulez-vous supprimer : **{label}** ?")
...
@st.dialog("➕ Inscriptions Admin")
def admin_force_inscription_dialog(at_id, titre, liste_adh, dict_adh):
    st.write(f"Gestion : **{titre}**")
    res = supabase.table("inscriptions").select("*, adherents(nom, prenom)").eq("atelier_id", at_id).execute()
    if res.data:
        for i in res.data:
            c1, c2 = st.columns([0.8, 0.2])
            nom = f"{i['adherents']['prenom']} {i['adherents']['nom']}"
            c1.write(f"• {nom} ({i['nb_enfants']} enf.)")
...
if menu == "📝 Inscriptions":
    ...
    if user != "Choisir...":
        ...
        for at in res_at.data:
            ...
            label = f"{format_date_fr_complete(at['date_atelier'])} — {at['titre']} | 📍 {at['lieux']['nom']} | {'🔒 Réservé' if is_locked else f'✅ {rest} pl.'}"
            
            with st.expander(label):
                if is_locked: st.warning("⚠️ Inscriptions gérées par l'administration.")
                ...
                if not is_locked:
                    ...
                    if c3.button("S'inscrire", key=f"v_{at['id']}", type="primary"):
                        if q != "Choisir..." and (rest - (1+e) >= 0):
                            ...
                            enregistrer_log(user, "Inscription", f"{q} (+{e}) le {at['date_atelier']}")
                            st.rerun()
                        elif q != "Choisir...": st.error("Plus de places disponibles")
...
# ==========================================
    ...
    with t1:
        sel = st.multiselect("Filtrer AM", liste_adh)
        ...
        if data.data:
            ...
            st.download_button("📥 Excel", data=export_to_excel(df), file_name="export_am.xlsx")
            curr = ""
    ...
    with t2:
        ...
        for a in ats.data:
            ...
            st.markdown(f"**{format_date_fr_complete(a['date_atelier'])}** | {a['titre']} <span class='compteur-badge'>👤 {t_am}</span> <span class='compteur-badge'>👶 {t_en}</span>", unsafe_allow_html=True)
...
# ==========================================
    ...
    if pw == current_code or pw == "0000":
        t1, t2, t3, t4, t5, t6, t7 = st.tabs(["🏗️ Ateliers", "📅 Gestion Planning", "📈 Stats", "👥 AM", "📍 Lieux/H", "⚙️ Code", "📜 Journal"])
        ...
        with t1: # ATELIERS
            ...
            if sub == "Générateur":
                ...
                if st.button("📊 Pré-remplir"):
                    tmp, curr = [], start
                    while curr <= end:
                        if ["Lundi", "Mardi", "Mercredi", "Jeudi", "Vendredi", "Samedi", "Dimanche"][curr.weekday()] in jours:
                            tmp.append({"Date": format_date_fr_complete(curr, False), "Titre": "", "Lieu": l_names[0], "Horaire": h_names[0], "Cap": 12, "🔒": False})
                        curr += timedelta(days=1)
                    st.session_state['at_list_gen'] = tmp
                
                if st.session_state['at_list_gen']:
                    df_ed = st.data_editor(pd.DataFrame(st.session_state['at_list_gen']), num_rows="dynamic", column_config={"Lieu": st.column_config.SelectboxColumn(options=l_names), "Horaire": st.column_config.SelectboxColumn(options=h_names)})
                    if st.button("💾 Créer les ateliers"):
                        for _, r in df_ed.iterrows():
                            supabase.table("ateliers").insert({"date_atelier": parse_date_fr_to_iso(r['Date']), "titre": r['Titre'], "lieu_id": next(l['id'] for l in l_raw if l['nom']==r['Lieu']), "horaire_id": next(h['id'] for h in h_raw if h['libelle']==r['Horaire']), "capacite_max": r['Cap'], "est_verrouille": r['🔒']}).execute()
                        st.session_state['at_list_gen'] = []; st.rerun()

            elif sub == "Répertoire":
                rep = supabase.table("ateliers").select("*, lieux(nom)").gte("date_atelier", date.today().strftime('%Y-%m-%d')).order("date_atelier").execute().data
                for a in rep:
                    c1, c2, c3, c4 = st.columns([0.6, 0.1, 0.1, 0.1])
                    c1.write(f"**{format_date_fr_complete(a['date_atelier'])}** | {a['titre']}")
                    # BOUTON VERROU (Nouveau)
                    v_icon = "🔒" if a.get('est_verrouille') else "🔓"
                    if c2.button(v_icon, key=f"lck_{a['id']}"):
                        supabase.table("ateliers").update({"est_verrouille": not a.get('est_verrouille', False)}).eq("id", a['id']).execute(); st.rerun()
                    # BOUTON ACTIF
                    a_icon = "🟢" if a['est_actif'] else "🔴"
                    if c3.button(a_icon, key=f"act_{a['id']}"):
        ...
        with t2: # GESTION PLANNING
            d_p = st.date_input("Planning du", date.today())
        ...
        with t3: # STATS
            ins_st = supabase.table("inscriptions").select("*, adherents(*)").execute()
            if ins_st.data:
                df_st = pd.DataFrame([{"AM": f"{i['adherents']['prenom']} {i['adherents']['nom']}"} for i in ins_st.data])
                st.table(df_st.value_counts().reset_index(name="Total Ateliers"))
        ...
        with t5: # LIEUX
            st.subheader("Lieux"); l_db = supabase.table("lieux").select("*").eq("est_actif", True).execute().data
            for l in l_db:
            ...
            with st.form("add_l"):
                nl = st.text_input("Nouveau Lieu"); cp = st.number_input("Capacité", 1, 50, 12)
                if st.form_submit_button("Ajouter Lieu"): supabase.table("lieux").insert({"nom": nl, "capacite_accueil": cp}).execute(); st.rerun()

        with t6: # CODE
            new_c = st.text_input("Nouveau code secret", type="password")
            if st.button("Changer le code"):
                supabase.table("configuration").update({"secret_code": new_c}).eq("id", "main_config").execute(); st.success("Code modifié")

        with t7: # JOURNAL
            dj1, dj2 = st.columns(2); d1 = dj1.date_input("Depuis", date.today()-timedelta(days=7)); d2 = dj2.date_input("Jusqu'à", date.today())
            res_l = supabase.table("logs").select("*").gte("created_at", str(d1)).lte("created_at", str(d2)+"T23:59:59").order("created_at", desc=True).execute()
