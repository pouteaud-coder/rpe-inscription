...
# ==========================================
    ...
    with t2:
        ...
        for index, a in enumerate(ats_raw.data):
            ...
            st.markdown(f"""
                **{format_date_fr_complete(a['date_atelier'])}** | <span class='lieu-badge' style='background-color:{c_l}'>{a['lieux']['nom']}</span> | 
...
# ==========================================
    ...
    if pw == current_code:
        ...
        with t_ad3: # LIEUX & HORAIRES
            ...
            with cl1:
                ...
                for l in l_raw:
                    ...
                    if c_b.button("🗑️", key=f"l_{l['id']}"): 
                        secure_delete_dialog("lieux", l['id'], l['nom'], current_code)
                with st.form("new_l"):
                    nl = st.text_input("Nouveau Lieu")
                    if st.form_submit_button("Ajouter"):
                        supabase.table("lieux").insert({"nom": nl, "est_actif": True, "capacite_accueil": 10}).execute()
                        st.rerun()
            with cl2:
                st.subheader("Horaires")
                for h in h_raw:
                    c_c, c_d = st.columns([0.85, 0.15])
                    c_c.write(f"• {h['libelle']}")
                    if c_d.button("🗑️", key=f"h_{h['id']}"): 
        ...
        st.info("Saisissez le code secret administrateur.")
